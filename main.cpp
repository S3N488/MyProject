...
void password()
{
	...
	while (c != '\r') //Проверка на нажатие Enter
	{
		c = _getch();
		if (c == 0)
		{
		...
		else if (c == '\b')   //Tckb Backspace нажат
		{
			if (password.size() != 0)  //Если строка пароля содержит данные, стереть последний символ
			{
				cout << "\b \b";
		...
		else if (c <= '9' && c >= '0' || c >= 'a' && c <= 'z' || c >= 'A' && c <= 'Z')  //Если пользователь вводит 1–9, az или AZ, добавьте его к паролю и отобразить звездочку.
		{
			password += c;
			cout << "*";
		}
...
void load()
{
	...
	for (r = 1; r <= timer; r++)
	{
		for (q = 0; q <= 100000000; q++);//отображать символ медленно
		printf("%c", 177);
	}
	Sleep(100);
	system("cls");
...
void newCustData()
{
	...
	cout << "\t  |\t\t\t\t\t  Phone Number : ";
	cin.getline(cust[custCount()].phone, 11);
